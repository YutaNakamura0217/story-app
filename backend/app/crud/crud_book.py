...
def create_book(db: Session, book: schemas.BookCreate) -> models.Book:
    ...
    db.add(db_book)
    db.flush()  # Flush to get db_book.id if generated by DB, instead of commit
    # db.commit()  # Commit to get db_book.id <-- Removed
    ...
    if book.theme_ids:
        ...
        # db.commit() # <-- Removed, all changes will be committed by the caller
        db.flush()  # Ensure BookTheme entries are flushed before refreshing db_book if relationships need it

    # Refresh to load all relationships and generated values
    db.refresh(db_book)
    return db_book
...
def update_book(db: Session, db_book: models.Book, book_in: schemas.BookUpdate) -> models.Book:
    ...
    db.add(db_book)  # or db.merge(db_book)
    # db.commit() # Removed
    db.flush()  # Ensure changes are sent to DB
    db.refresh(db_book)
    return db_book
...
def delete_book(db: Session, db_book: models.Book) -> models.Book:
    db.delete(db_book)
    # db.commit() # Removed
    db.flush()  # Ensure delete is sent to DB
    return db_book
...
def update_book_page(db: Session, db_page: models.BookPage, page_in: schemas.BookPageUpdate) -> models.BookPage:
    ...
    db.add(db_page)  # or db.merge(db_page)
    # db.commit() # Removed
    db.flush()
    db.refresh(db_page)
    return db_page
...
def update_book_toc_item(db: Session, db_toc_item: models.BookTocItem, toc_item_in: schemas.BookTocItemUpdate) -> models.BookTocItem:
    ...
    db.add(db_toc_item)  # or db.merge(db_toc_item)
    # db.commit() # Removed
    db.flush()
    db.refresh(db_toc_item)
    return db_toc_item
