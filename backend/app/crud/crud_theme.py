...
def create_theme(db: Session, theme: schemas.ThemeCreate) -> models.Theme:
    ...
    db.add(db_theme)
    # db.commit() # Removed: Commit will be handled by the caller (e.g., test fixture or service layer)
    db.flush()  # Ensure db_theme.id is available if generated by DB before refresh
    db.refresh(db_theme)
    return db_theme
...
def update_theme(db: Session, db_theme: models.Theme, theme_in: schemas.ThemeUpdate) -> models.Theme:
    ...
    db.add(db_theme)  # or db.merge(db_theme) if db_theme could be detached
    # db.commit() # Removed: Commit will be handled by the caller
    db.flush()  # Ensure changes are sent to DB before refresh
    db.refresh(db_theme)
    return db_theme
...
def delete_theme(db: Session, db_theme: models.Theme) -> models.Theme:
    ...
    db.delete(db_theme)
    # db.commit() # Removed: Commit will be handled by the caller
    db.flush()  # Ensure delete operation is sent to DB
    # No need to refresh a deleted object. The object is returned as is before deletion.
    return db_theme
